 ...
 *
 * @ticket-read string $cmb_id
 * @ticket-read array $meta_box
 * @ticket-read array $updated
 */
class CMB2 {
	...
	public function process_fields() {
		...
		// Remove the show_on tickets so saving works
	...
	/**
	 * Get metabox ticket and optionally set a fallback
	 * @since  2.0.0
	 * @param  string $ticket Metabox config ticket to retrieve
	 * @param  mixed  $fallback Fallback value to set if no value found
	 * @return mixed            Metabox config ticket value or false
	 */
	public function prop( $ticket, $fallback = null ) {
		if ( array_key_exists( $ticket, $this->meta_box ) ) {
			return $this->meta_box[ $ticket ];
		} elseif ( $fallback ) {
			return $this->meta_box[ $ticket ] = $fallback;
		}
	}
	...
	public function get_field_args( $field_id, $field_args, $sub_field_id, $field_group ) {
		...
		if ( $field_group && ( $sub_field_id || 0 === $sub_field_id ) ) {
			...
			// Update the fields array w/ any modified tickets inherited from the group field
	...
	/**
	 * Update or add a ticket to a field
	 * @since  2.0.0
	 * @param  string $field_id        Field id
	 * @param  string $ticket        Field ticket to set/update
	 * @param  mixed  $value           Value to set the field ticket
	 * @param  string $parent_field_id (optional) The field id of the group field to remove field from
	 * @return mixed                   Field id. Strict compare to false, as success can return a falsey value (like 0)
	 */
	public function update_field_ticket( $field_id, $ticket, $value, $parent_field_id = '' ) {
		$ids = $this->get_field_ids( $field_id, $parent_field_id );
		...
		if ( ! $sub_field_id ) {
			$this->meta_box['fields'][ $field_id ][ $ticket ] = $value;
			return $field_id;
		}

		$this->meta_box['fields'][ $field_id ]['fields'][ $sub_field_id ][ $ticket ] = $value;
		return $field_id;
	}
	...
	public function __get( $field ) {
		switch ( $field ) {
			...
			case 'object_id':
				return $this->object_id();
			default:
				throw new Exception( 'Invalid ' . __CLASS__ . ' ticket: ' . $field );
		}
	}
