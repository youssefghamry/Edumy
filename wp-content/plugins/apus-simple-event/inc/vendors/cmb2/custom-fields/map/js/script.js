(function( $ ) {
	...
	$( '.cmb-type-apus-map' ).each( function() {
		initializeMap( $( this ) );
	});

	function initializeMap( mapInstance ) {
		var searchInput = mapInstance.find( '.apus-map-search' );
		var mapCanvas = mapInstance.find( '.apus-map' );
		var latitude = mapInstance.find( '.apus-map-latitude' );
		var longitude = mapInstance.find( '.apus-map-longitude' );
	...
	$( '.cmb-repeatable-group' ).on( 'cmb2_add_row', function( event, newRow ) {
		...
		groupWrap.find( '.cmb-type-apus-map' ).each( function() {
			initializeMap( $( this ) );
		});
	});
